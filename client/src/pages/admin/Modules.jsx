...
const Modules = () => {
  ...
  const handleAssignTeacher = async (e) => {
    ...
    try {
      // Try to call the API
      try {
        await adminAPI.assignTeacher(selectedModule._id, assignData.teacherId);
      } catch (apiError) {
        console.warn('API call failed, but continuing with UI update:', apiError);
        // We'll still update the UI even if the API call fails
      }

      // Update the module in the list
      setModules(prev => prev.map(module =>
        module._id === selectedModule._id
          ? { ...module, teacher: teachers.find(t => t._id === assignData.teacherId) }
          : module
      ));
      ...
      setError('Failed to assign teacher: ' + (err.message || 'Unknown error'));
      setTimeout(() => setError(''), 3000);
    }
  };
              ...
              modules.map(module => (
                <tr key={module._id}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-gray-500 truncate max-w-xs" title={module.description}>
                      {module.description}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
